...
// arche/components/only-home/home.component.ts
...
// arche/components/only-navbar/navbar.component.ts
...
// arche/components/only-login/login.component.ts
...
// arche/components/only-register/register.component.ts
...
// arche/components/only-feed/feed.component.ts
...
// arche/components/only-article/article.component.ts
class OnlyArticle extends HTMLElement {
  ...
  async loadArticle(token, id, isStaff) {
    ...
    try {
      ...
      this.querySelector("#save-article-btn")?.addEventListener("click", async () => {
        ...
        const contentType = this.querySelector("#edit-type").value;
        const body = this.querySelector("#edit-body").value.trim();
        if (!title || !body) {
...
// arche/components/only-chat/chat.component.ts
  ...
  async loadMessages(token) {
    ...
    try {
      ...
      messages.forEach((m) => {
        ...
        const otherName = m.sender_id === myId ? m.receiver_username ?? `Usuario #${m.receiver_id}` : m.sender_username ?? `Usuario #${m.sender_id}`;
...
// arche/components/only-store/store.component.ts
class OnlyStore extends HTMLElement {
  async connectedCallback() {
    ...
    if (isStaff) {
      this.querySelector("#new-product-btn")?.addEventListener("click", () => {
        const form = this.querySelector("#new-product-form");
        form.style.display = form.style.display === "none" ? "block" : "none";
      });
  ...
  clearProductForm() {
    this.querySelector("#product-name").value = "";
    this.querySelector("#product-description").value = "";
    this.querySelector("#product-price").value = "";
    this.querySelector("#product-stock").value = "";
  }
  async saveProduct(token) {
    const name = this.querySelector("#product-name").value.trim();
    const description = this.querySelector("#product-description").value.trim();
    const price = parseFloat(this.querySelector("#product-price").value);
    const stock = parseInt(this.querySelector("#product-stock").value);
  ...
  async loadProducts(token, isStaff) {
      ...
      if (isStaff) {
        ...
        this.querySelectorAll(".save-edit-btn").forEach((btn) => {
          btn.addEventListener("click", async () => {
            ...
            const res2 = await fetch(`http://localhost:3001/products/${id}`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
              body: JSON.stringify({ name, description, price, stock })
            });
...
// arche/components/only-stream/stream.component.ts
...
// arche/components/only-profile/profile.component.ts
...
// arche/components/only-admin/admin.component.ts
...
// arche/components/only-error/error.component.ts
var ERROR_CONFIG = {
  "404": {
    code: "404",
